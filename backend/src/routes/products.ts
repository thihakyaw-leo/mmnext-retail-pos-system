...
import { edgeCacheMiddleware } from '../middleware/cache.js';
...
async function updateProductStock(db, productId, quantityChange, userId, orgId, logData) {
  const product = await db.prepare(`
    SELECT stock_quantity FROM products WHERE id = ? AND organization_id = ?
  `).bind(productId, orgId).first()
  ...
  `).bind(newQuantity, productId, orgId).run()
...
products.get('/', edgeCacheMiddleware(60), async (c) => {
  try {
...
products.get('/:id', edgeCacheMiddleware(60), async (c) => {
  try {
    ...
    `).bind(productId, c.get('orgId')).first()
...
products.get('/barcode/:barcode', async (c) => {
  try {
    ...
    const product = await c.env.DB.prepare(`
      ...
      FROM products p
      LEFT JOIN categories c ON p.category_id = c.id
      WHERE p.barcode = ? AND p.is_active = 1 AND p.organization_id = ?
    `).bind(barcode, c.get('orgId')).first()
...
products.get('/low-stock', edgeCacheMiddleware(60), async (c) => {
  try {
    const lowStockProducts = await c.env.DB.prepare(`
      ...
      WHERE p.stock_quantity <= p.reorder_level 
        AND p.is_active = 1 AND p.organization_id = ?
...
products.post('/', async (c) => {
  try {
    ...
    `).bind(validatedData.sku, orgId).first()
    ...
    if (validatedData.barcode) {
      const existingBarcode = await c.env.DB.prepare(`
        SELECT id FROM products WHERE barcode = ? AND organization_id = ?
      `).bind(validatedData.barcode, orgId).first()
    ...
    await c.env.DB.prepare(`
      INSERT INTO products (
        id, organization_id, sku, name, description, category_id, price, cost_price,
  ...
  try {
    ...
    if (validatedData.sku && validatedData.sku !== existingProduct.sku) {
      const skuExists = await c.env.DB.prepare(`
        SELECT id FROM products WHERE sku = ? AND id != ? AND organization_id = ?
      `).bind(validatedData.sku, productId, orgId).first()
    ...
    if (validatedData.barcode && validatedData.barcode !== existingProduct.barcode) {
      const barcodeExists = await c.env.DB.prepare(`
        SELECT id FROM products WHERE barcode = ? AND id != ? AND organization_id = ?
      `).bind(validatedData.barcode, productId, orgId).first()
